...
/// A scrollable grid that automatically animates its children.
///
/// [MotionGridView] provides two constructors:
///
/// * The default constructor takes explicit [children] and wraps them in
///   a scrollable [ScrollAwareMotionLayout]. Best for small-to-medium grids
///   where full FLIP animations are desired.
///
/// * [MotionGridView.builder] creates items lazily using [itemBuilder],
///   similar to [GridView.builder]. It requires [keyBuilder] for diff-based
///   animation. Enter animations play when items scroll into view.
///
/// {@tool snippet}
/// ```dart
/// MotionGridView(
...
/// ```
/// {@end-tool}
class MotionGridView extends StatefulWidget {
  /// Creates a scrollable animated grid from explicit [children].
  ///
  /// All children must have unique [Key]s.
  const MotionGridView({
    super.key,
  ...
  /// Key builder for each item index (builder constructor).
  ...
  @override
  State<MotionGridView> createState() => _MotionGridViewState();
}

class _MotionGridViewState extends State<MotionGridView> {
  // Builder mode state.
  ...
  @override
  void didUpdateWidget(covariant MotionGridView oldWidget) {
  ...
  // ---------------------------------------------------------------------------
  // Children mode — wraps GridView in ScrollAwareMotionLayout
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // Builder mode — SliverGrid with animation wrappers
  // ---------------------------------------------------------------------------
